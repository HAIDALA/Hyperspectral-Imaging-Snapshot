function [I_WNMF_rec,F_pool,Final_norm,I_WNMF_err] = WNMF_Demosaicing_With_Analysis(I_MOS_seq,SMP_seq,FilterPattern_Cell,num_band,WNMF_params,num_obs_pxl,I_WB,smp_scenario)
...
for tt=1:numel(FilterPattern_lst)
    ...
    if(I_WB_Init) % If we need to initialize using Weighted Bilinear Interpolaztion
        ...
        [D_rec0{tt}]=D_rec0{tt}';
        [Error{tt}]=repmat(f,[num_band,num_band]);
...
elseif WNMF_params.Kmeans_cut==5
    ...
    mean_F_pool=Final_norm_temp(100);
    
end
j=1;
pure=repmat(200,[num_band,num_band]);
non_pure=repmat(10,[num_band,num_band]);
...
for tt=1:numel(FilterPattern_lst)
    ...
    tmp2=reshape(Error{tt},[offset,offset,num_band]);
    ...
    I_rec1(Loc{tt}(1):Loc{tt}(1)+offset-1,Loc{tt}(2):Loc{tt}(2)+offset-1,:)=tmp2+I_rec1(Loc{tt}(1):Loc{tt}(1)+offset-1,Loc{tt}(2):Loc{tt}(2)+offset-1,:);
end
I_WNMF_rec=I_rec0./CNT;
I_WNMF_err=I_rec1./CNT;
