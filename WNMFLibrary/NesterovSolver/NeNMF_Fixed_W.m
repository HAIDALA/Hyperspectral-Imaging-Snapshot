function [W,H,iter,elapse,HIS,V]=NeNMF_Fixed_W(V,r,varargin)
...
tol=1e-5;
verbose=0;
% This code is added by Kinan on 10 Jan 2022 to control the sum to one
% constrain
scalling=false;
...
if (rem(length(varargin),2)==1)
    ...
    for i=1:2:(length(varargin)-1)
        switch upper(varargin{i})
            ...
            % This code is added by Kinan on 10 Jan 2022 to control the sum to one
            % constrain
            case 'SCALLING',    scalling=varargin{i+1};
            % This code is added to let the NMF work with W fixed
            case 'W_FFIXID',    W_Fixed=varargin{i+1};
            otherwise
...
%     %% This section is added by Kinan ABBAS - 8 Dec 2021
%     % Apply sum to one constraint on the columns of the H
    if(scalling==true)
      H=ScaleColumns(H);
    end
    if iterH<=ITER_MIN,
...
%     GradW=0;
    ...
    delta=GetStopCriterion(STOP_RULE,[H],[GradH]);
