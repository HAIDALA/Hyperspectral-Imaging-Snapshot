...
% Run the expirement
if(With_Noise==true)
    [mean_PSNR,mean_SAM,exec_time,std_PSNSR,std_SAM,mean_MSE,mean_SIR,mean_MER,mean_U_PSNR,mean_SSIM,mean_RMSE,mean_MRSA]=Evaluate_on_complex_image_noisy(num_band,sz,smp_scenario,num_obs_pxl,GRMR_params,WNMF_params,unmixing,num_of_experiments);
    
else
    [mean_PSNR,mean_SAM,exec_time,std_PSNSR,std_SAM,mean_MSE,mean_SIR,mean_MER,mean_U_PSNR,mean_SSIM,mean_RMSE,mean_MRSA]=Evaluate_on_complex_image(num_band,sz,smp_scenario,num_obs_pxl,GRMR_params,WNMF_params,unmixing,num_of_experiments);
end
% Showing the results
fprintf('\n');
...
fprintf('Naive | %.1f   | %.1f   | %.2f | %.2f      | %.2f | %.2f | %.2f | %.2f | %.2f | %.2f \n',mean_U_PSNR(8),mean_PSNR(8),mean_SAM(8),mean_MSE(8),mean_RMSE(8),mean_SSIM(8),mean_SIR(8),mean_MER(8),exec_time(8),mean_MRSA(8));