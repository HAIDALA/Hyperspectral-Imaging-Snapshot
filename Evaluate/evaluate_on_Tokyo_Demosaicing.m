function [mean_PSNR]=evaluate_on_Tokyo_Demosaicing(num_band,sz,smp_scenario,num_obs_pxl,GRMR_params,WNMF_params,dataset_size)
...
% fnames=dir('C:\Users\Kinan\Desktop\Library\Datasets\TokyoTech 59 bands dataset\');
fnames=dir('Data/TokyoTech 59 bands dataset');
for zz=1:dataset_size
    ...
    I_HS=load(fname);
    I_HS=I_HS.img;
    I_HS=I_HS(1:sz(1),1:sz(2),1:num_band);
...
%     fprintf('Naive  | %.1f   \n',mean(err_WNMF1(zz,:)));
    ...
    save('Results/'+string(fname),'WNMF','I_HS','I_GRMR_rec','I_WB','I_PPID','I_ItSD','I_BTES');
